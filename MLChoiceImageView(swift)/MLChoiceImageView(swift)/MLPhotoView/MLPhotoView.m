...
@interface MLPhotoView () <UIScrollViewDelegate>
    ...
    if (self) {
        ...
        if (HW > 3 || WH > 3) {
            if (HW != 0) {
                ...
                CGFloat W = 0;
                CGFloat H = 0;
                B = [UIScreen mainScreen].bounds.size.width/image.size.width  ;
                W = [UIScreen mainScreen].bounds.size.width;
                H = image.size.height * B;
...
- (void)setupGestureRecognizer
{
    ...
    //单击
    UITapGestureRecognizer *singleTapGesture = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(danji:)];
...
-(void)danji:(UITapGestureRecognizer *)recognizer
{
    //创建单击通知
    NSNotificationCenter *center = [NSNotificationCenter defaultCenter];
    [center postNotificationName:@"MLPhotoBrowser_SingleClick" object:self];
}
...
////    CGFloat maxScale = MAX(imageSize.height / imagePresentationSize.height, imageSize.width / imagePresentationSize.width);
    self.maximumZoomScale = 4.0; // Should not less than 1
